...
def plot_clusters_with_hubs_satellites(data, centroids, satellite_locations, hub_locations, labels):
    plt.figure(figsize=(10, 6))
    ...
    plt.title('Clusters with Satellites and Hubs')
    plt.xlabel('X')
    plt.ylabel('Y')
    plt.grid(True)
    plt.legend()
...
plot_elbow(inertias_A, k_range_A)
...
plot_elbow(inertias_B, k_range_B)
...
plot_clusters_with_hubs_satellites(coordinates_districtA[['X', 'Y']], centroids_A, satellite_locations_A, hub_locations_A, labels_A)
...
plot_clusters_with_hubs_satellites(coordinates_districtB[['X', 'Y']], centroids_B, satellite_locations_B, hub_locations_B, labels_B)
...
deltaT= 5 #time variable 

# Funzione obiettivo per il distretto A
obj_A = gp.quicksum((dist_pickupsA_satellitesA[i, j] / Vb)*deltaT * x_A[i, j] for i in range(num_customers_per_district) for j in range(num_satellites_per_cluster * k_optimal_districtA)) + gp.quicksum(gk_A * zk_A[k] for k in range(num_satellites_per_cluster * k_optimal_districtA))
# Aggiunta dei percorsi satellite A -> hub A
# Aggiunta dei percorsi satellite A -> hub A
obj_A += gp.quicksum((dist_satellitesA_hubA[i, j] / Vt)*deltaT * w_A[i, j] for i in range(num_satellites_per_cluster * k_optimal_districtA) for j in range(num_hubs_per_cluster * k_optimal_districtA))
# Aggiunta dei percorsi hub A -> hub B
obj_A += gp.quicksum((dist_hubA_hubB[i, j] / Vt)*deltaT * h_A[i] * h_B[j] for i in range(num_hubs_per_cluster * k_optimal_districtA) for j in range(num_hubs_per_cluster * k_optimal_districtB))
# Aggiunta dei percorsi hub B -> satellite B
obj_A += gp.quicksum((dist_hubB_satellitesB[i, j] / Vt)*deltaT * h_B[i] * zk_B[j] for i in range(num_hubs_per_cluster * k_optimal_districtB) for j in range(num_satellites_per_cluster * k_optimal_districtB))
# Aggiunta dei percorsi satellite B -> delivery B
obj_A += gp.quicksum((dist_satellitesB_deliveriesB[i, j] / Vb)*deltaT * zk_B[i] for i in range(num_satellites_per_cluster * k_optimal_districtB) for j in range(num_customers_per_district))


# Definizione del vettore di traslazione (traslazione di 10 unità verso il basso e a destra)
translation_vector = np.array([300, -300])

# Funzione obiettivo per il distretto B con traslazione
obj_B = (
    gp.quicksum((dist_pickupsB_satellitesB[i, j] / Vb) * deltaT * x_B[i, j] for i in range(num_customers_per_district) for j in range(num_satellites_per_cluster * k_optimal_districtB)) +
    gp.quicksum(gk_B * zk_B[k] for k in range(num_satellites_per_cluster * k_optimal_districtB)) +
    gp.quicksum((dist_satellitesB_hubB[i, j] / Vt) * deltaT * w_B[i, j] for i in range(num_satellites_per_cluster * k_optimal_districtB) for j in range(num_hubs_per_cluster * k_optimal_districtB)) +
    gp.quicksum((dist_hubB_hubA[i, j] / Vt) * deltaT * h_B[i] * h_A[j] for i in range(num_hubs_per_cluster * k_optimal_districtB) for j in range(num_hubs_per_cluster * k_optimal_districtA)) +
    gp.quicksum((dist_hubB_satellitesB[i, j] / Vt) * deltaT * h_B[i] * zk_B[j] for i in range(num_hubs_per_cluster * k_optimal_districtB) for j in range(num_satellites_per_cluster * k_optimal_districtB)) +
    gp.quicksum((dist_satellitesB_deliveriesB[i, j] / Vb) * deltaT * zk_B[i] for i in range(num_satellites_per_cluster * k_optimal_districtB) for j in range(num_customers_per_district)) +
    # Termine di penalità basato sulla distanza tra le coordinate dei punti nel distretto B e il vettore di traslazione
    gp.quicksum(np.linalg.norm(np.array([pickup_coordsB.iloc[i]['X'], pickup_coordsB.iloc[i]['Y']]) - translation_vector) for i in range(len(pickup_coordsB))) +
    gp.quicksum(np.linalg.norm(np.array([delivery_coordsB.iloc[i]['X'], delivery_coordsB.iloc[i]['Y']]) - translation_vector) for i in range(len(delivery_coordsB)))
)
...
for j in range(num_satellites_per_cluster * k_optimal_districtA):
    m.addConstr(gp.quicksum(x_A[i, j] for i in range(num_customers_per_district)) <= capacity_truck)

# Vincoli di capacità per gli hub nel distretto A
for j in range(num_hubs_per_cluster * k_optimal_districtA):
    m.addConstr(gp.quicksum(z_A[j, i] for i in range(num_customers_per_district)) <= 1000)
...
for i in range(num_customers_per_district):
    m.addConstr(gp.quicksum(z_A[j, i] for j in range(num_hubs_per_cluster * k_optimal_districtA)) == 1)

# Vincoli di capacità per i satelliti nel distretto B
for j in range(num_satellites_per_cluster * k_optimal_districtB):
    m.addConstr(gp.quicksum(x_B[i, j] for i in range(num_customers_per_district)) <= capacity_truck)

# Vincoli di capacità per gli hub nel distretto B
for j in range(num_hubs_per_cluster * k_optimal_districtB):
    m.addConstr(gp.quicksum(z_B[j, i] for i in range(num_customers_per_district)) <= 1000)
...
for i in range(num_customers_per_district):
    m.addConstr(gp.quicksum(z_B[j, i] for j in range(num_hubs_per_cluster * k_optimal_districtB)) == 1)
...
# Vincoli per garantire che solo un hub sia attivo per distretto A
for dis in range(1):
    m.addConstr(
        gp.quicksum(h_A[dis * num_hubs_per_cluster + k] for k in range(num_hubs_per_cluster)) == 1,
        f"cluster_{dis}_hub_A"
    )

# Vincoli per garantire che solo un hub sia attivo per distretto B
for dis in range(1):
    m.addConstr(
        gp.quicksum(h_B[dis * num_hubs_per_cluster + k] for k in range(num_hubs_per_cluster)) == 1,
        f"cluster_{dis}_hub_B"
    )
