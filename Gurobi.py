...
num_satellites_per_cluster = 200
...
obj_A = gp.quicksum(dist_pickup_to_satellite_A[i, j] / Vb * x_A[i, j] for i in range(num_customers_per_district) for j in range(num_satellites_per_cluster * k_optimal_districtA)) + gp.quicksum(gk_A * zk_A[k] for k in range(num_satellites_per_cluster * k_optimal_districtA))

# Funzione obiettivo per il distretto B
obj_B = gp.quicksum(dist_delivery_to_satellite_B[i, j] / Vb * x_B[i, j] for i in range(num_customers_per_district) for j in range(num_satellites_per_cluster * k_optimal_districtB)) + gp.quicksum(gk_B * zk_B[k] for k in range(num_satellites_per_cluster * k_optimal_districtB))
